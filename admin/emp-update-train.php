<?php
    ...
    if(isset($_POST['update_train']))
    {
            ...
            $query="update orrs_train set train_name= ?, train_no= ?, journey_date = ?, route=?,deptime = ?, arrtime = ?, fare = ?, passengers = ? where id = ?";
            $stmt = $mysqli->prepare($query);
            //bind this parameters
            $rc=$stmt->bind_param('ssssssssi',$train_name,$train_no,$journey_date,$route,$deptime,$arrtime,$fare, $passengers,$id);
            $stmt->execute();
                if($stmt)
                {
                    $succ = "Train Updated";
