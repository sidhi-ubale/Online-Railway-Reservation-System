<?php
    ...
    if(isset($_POST['train_fare_confirm_checkout']))
    {
            ...
            $query="insert into orrs_train_tickets (pass_name, pass_email,train_name, train_no, deptime,arrtime,  fare, fare_payment_code) values (?,?,?,?,?,?,?,?)";
            $stmt = $mysqli->prepare($query); //prepare sql and bind it later
            $rc=$stmt->bind_param('ssssssss', $pass_name, $pass_email ,$train_name, $train_no, $deptime, $arrtime, $fare, $fare_payment_code);
            $stmt->execute();
            if($stmt)
            {
