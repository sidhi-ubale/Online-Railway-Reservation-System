<?php
    ...
    if(isset($_POST['Book_Train']))
    {
            ...
            */
            ...
            $joureny_date= $_POST['journey_date'];
            $route = $_POST['route'];
            $deptime = $_POST['deptime'];
            $arrtime = $_POST['arrtime'];
            $fare = $_POST['fare'];
            //sql file to update the table of passengers with the new captured information
            $query="update  orrs_passenger set train_no = ?, train_name = ?,journey_date = ?,route=?,deptime = ?,arrtime = ?, fare = ? where pass_id=?";
            $stmt = $mysqli->prepare($query); //prepare sql and bind it later
            $rc=$stmt->bind_param('sssssssi', $train_no, $train_name, $journey_date,$route ,$deptime, $arrtime, $fare, $aid);
            $stmt->execute();
            if($stmt)
            {
                $succ = "Ticket Reserved! Please Proceed To Check Out";
            }
            else 
            {
                ...
                <div class="card-body">
                  <form method ="POST">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" readonly name="journey_date" value="<?php echo $row->journey_date;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3">Route</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" readonly name="route" value="<?php echo $row->route;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" readonly name="deptime" value="<?php echo $row->deptime;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label class="col-12 col-sm-3 col-form-label text-sm-right" for="inputText3">Arrival Time</label>
                      <div class="col-12 col-sm-8 col-lg-6">
                        <input class="form-control" readonly name="arrtime" value="<?php echo $row->arrtime;?>" id="inputText3" type="text">
                      </div>
                    </div>
                    <div class="form-group row">
